  ...
  <section data-testid="instagram-strip" className="py-20 overflow-hidden bg-white">
    <Reveal>
      <div className="text-center mb-10">
        <div className="text-[0.7rem] tracking-[0.4em] text-[#2A7E7C] uppercase font-ui mb-3">The Atelier</div>
        <h2 className="font-display text-4xl md:text-6xl text-[#1A2E2E]">The Art, As It <em className="text-[#2A7E7C] italic">Leaves Our Hands</em></h2>
      </div>
    </Reveal>
      ...
      <div className="flex gap-4 w-[200%] animate-marquee" style={{ willChange: "transform" }}>
        {STRIP.map((src, i) => (
          <a key={i} href="https://instagram.com" target="_blank" rel="noreferrer" className="relative shrink-0 w-60 h-60 overflow-hidden group rounded-2xl">
            ...
            <div className="absolute inset-0 ring-1 ring-[#C8DEDD] rounded-2xl" />
            <div className="absolute inset-0 bg-[#FFFFFF]/85 opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex items-center justify-center">
              <Instagram className="text-[#2A7E7C]" size={28} />
            </div>
          </a>
        ))}
      </div>
