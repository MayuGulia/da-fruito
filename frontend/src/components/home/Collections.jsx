...
export const Collections = () => {
  ...
  return (
    <section id="collections" data-testid="collections-section" className="py-24 md:py-32 relative overflow-hidden">
      <div className="lux-container">
        <Reveal>
          <div className="text-center mb-16">
            <div className="text-[0.7rem] tracking-[0.4em] text-bronze uppercase font-ui mb-3">Signature Hampers</div>
            <h2 className="font-display text-5xl md:text-7xl text-ivory">Our Signature <em className="text-antique italic">Collections</em></h2>
            ...
            {filtered.map((h, i) => (
              ...
              >
                <div className="relative h-72 overflow-hidden">
                  ...
                  <div className="absolute top-4 left-4 text-[0.62rem] italic font-body text-berry bg-ceramic px-3 py-1">{OCC_LABEL[h.occasion]}</div>
                </div>
                <div className="p-6">
                  <div className="font-display text-2xl text-ivory group-hover:text-antique transition-colors">{h.name}</div>
                  <p className="font-body text-ivory/60 italic text-sm mt-1 line-clamp-2">{h.description}</p>
                  <div className="flex flex-wrap gap-2 mt-4">
                    {h.materials?.slice(0, 3).map((m) => (
                      <span key={m} className="text-[0.62rem] font-ui uppercase tracking-[0.2em] text-antique border border-gold/40 px-2.5 py-1 rounded-full">{m}</span>
                    ))}
                  </div>
                  <div className="mt-5 flex items-center justify-between">
                    <div className="font-body text-ivory">From <span className="font-display text-2xl text-antique">{formatINR(h.price)}</span></div>
                    <button
        ...
        {open && (
          ...
          >
            ...
            <motion.div
              ...
              className="relative w-full max-w-4xl glass-light grid md:grid-cols-2"
            >
              <button data-testid="hamper-modal-close" onClick={() => setOpen(null)} className="absolute top-4 right-4 text-walnut z-10"><X size={22} /></button>
              <img src={open.image} alt={open.name} className="w-full h-72 md:h-full object-cover" />
              <div className="p-10">
                <div className="text-[0.65rem] tracking-[0.35em] text-berry italic uppercase mb-2">{OCC_LABEL[open.occasion]}</div>
                <h3 className="font-display text-4xl text-walnut mb-3">{open.name}</h3>
                <p className="font-body text-walnut/70 mb-6">{open.description}</p>
                <div className="mb-6">
                  <div className="text-[0.65rem] tracking-[0.3em] uppercase font-ui text-bronze mb-2">Contains</div>
                  <ul className="font-body text-walnut/80 space-y-1">
                    {open.items?.map((it) => <li key={it}>— {it}</li>)}
                  </ul>
                </div>
                <div className="font-display text-3xl text-walnut mb-6">{formatINR(open.price)}</div>
